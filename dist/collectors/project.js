 ...
 */
function countFilesInTree(cwd, filenames, checkCodexDir = true) {
    let count = 0;
    let currentDir = cwd;
 ...
 */
export function hasCodexDir(cwd) {
 ...
 */
export function countConfigFiles(cwd) {
    const codexDir = path.join(cwd, '.codex');
    if (!fs.existsSync(codexDir) || !fs.statSync(codexDir).isDirectory()) {
        return 0;
    }
    try {
        ...
        return files.filter(f => f.endsWith('.toml') ||
            f.endsWith('.json') ||
            f === 'config' ||
            f === 'settings').length;
    }
 ...
 */
export function collectProjectInfo(cwd, config) {
 ...
 */
export function formatProjectPath(cwd, maxLength = 30) {
