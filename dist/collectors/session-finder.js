 ...
 */
function findRolloutsInDir(dirPath) {
    const results = [];
 ...
 */
export function findMostRecentRollout(maxDaysBack = 7) {
 ...
 */
export function findActiveRollouts(withinSeconds = 60) {
    const sessionsDir = getSessionsDir();
    if (!fs.existsSync(sessionsDir)) {
        return [];
    }
 ...
 */
export function findRolloutBySessionId(sessionId, maxDaysBack = 7) {
    const sessionsDir = getSessionsDir();
    if (!fs.existsSync(sessionsDir)) {
        return null;
    }
...
//# sourceMappingURL=session-finder.js.map